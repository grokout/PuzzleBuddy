...
public class LeanTranslationNameDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		var color = GUI.color;
		var exists = LocalizationManager.HasKey(property.stringValue);
		...
		if (GUI.Button(right, "List") == true)
		{
			...
			if (string.IsNullOrEmpty(property.stringValue) == false)
			{
				if (exists == true)
				{
					...
					if (LeanLocalization.CurrentTranslations.TryGetValue(property.stringValue, out translation) == true)
					{
						...
						{
							var owner = entry.Owner; menu.AddItem(new GUIContent("Select/" + entry.Language), false, () => 
							{ 
			...
			foreach (string translationName in LocalizationManager.instance.termsChache.Keys)
			{
